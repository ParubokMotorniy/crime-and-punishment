...
class Inspector():
    ...
    def if_inspect_bounded_learning(self) -> bool:
        ...
        estimated_reward_probability = (simulation_constants.rationality_weight * self.rational_detection_estimate + simulation_constants.randomness_weight * random_guess_probability_estimate_noise)
        ...
        return estimated_reward_probability * float(simulation_constants.r) > simulation_constants.k

    def if_inspect_bounded_decision_making(self) -> bool:
        random_guess_probability_estimate_noise = np.random.uniform(-1.0,1.0)

        rational_payoff_estimate = self.rational_detection_estimate * simulation_constants.r - simulation_constants.k
        standardization_scale = (simulation_constants.r - simulation_constants.k) if rational_payoff_estimate > 0 else (- simulation_constants.k) #reward is always larger then inspection cost
        return simulation_constants.rationality_weight * float((abs(rational_payoff_estimate) / float(standardization_scale))) + simulation_constants.randomness_weight * random_guess_probability_estimate_noise > 0 
    ...
    def update_payoff(self, if_i_inspected:bool, if_my_inspectee_commited_crime:bool):
        ...
        inspection_gain = simulation_constants.r if if_i_inspected and if_my_inspectee_commited_crime else 0
